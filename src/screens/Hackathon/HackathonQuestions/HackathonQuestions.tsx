...
export function HackathonQuestions({ initialContext, onComplete, onBack }: HackathonQuestionsProps) {
  ...
  const handleOptionSelect = (questionId: string, value: string) => {
    if (questionId === 'team') {
      ...
      setContext((prev: Partial<HackathonContext>) => ({
        ...prev,
        team: {
          size: teamSize,
          members: prev.team?.members || [],
        },
      }));
    } else if (questionId === 'skills') {
      const currentSkills = context.team?.members.map((m: TeamMember) => m.role) || [];
      const newSkills = currentSkills.includes(value as any)
        ? currentSkills.filter((s: string) => s !== value)
        : [...currentSkills, value as any];
      
      // Create team members based on selected skills
      const members: TeamMember[] = newSkills.map((skill: string) => ({
      ...
      if (key) {
        setContext((prev: Partial<HackathonContext>) => ({
          ...
          submission: {
            ...prev.submission!,
            [key]: !prev.submission![key],
          } as HackathonContext['submission'],
        }));
      }
  ...
  const getSelectedValue = (questionId: string, optionValue: string): boolean => {
    if (questionId === 'team') {
      ...
      return context.team?.members.some((m: TeamMember) => m.role === optionValue) || false;
    } else if (questionId === 'timeline') {
  ...
  const getSkillGapWarning = (): string | null => {
    const roles = context.team?.members.map((m: TeamMember) => m.role) || [];
    
    if (roles.length === 0) return null;
    
    const hasFrontend = roles.some((r: string) => r === 'frontend' || r === 'fullstack');
    const hasBackend = roles.some((r: string) => r === 'backend' || r === 'fullstack');
